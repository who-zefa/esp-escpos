...
)
{
    ...
    for (uint32_t y = 0; y < height; y++) {
        ...
        for (uint32_t x = 0; x < width; x++) {
            ...
            int16_t value = (int16_t)gray + error;
            
            uint8_t bit = (value < threshold) ? 1 : 0;
...
)
{
    ...
    image->print_area_width = params->max_width ? params->max_width : ESCPOS_MAX_IMAGE_WIDTH;
...
)
{
    ...
    uint8_t header[8];
    uint8_t align_cmd[] = {0x1B, 0x61, (uint8_t)image->align};
    uint8_t pos_cmd[] = {0x1B, 0x24, 0x00, 0x00};  /* ESC $ nL nH */
    ...
    if (area_width > effective_width) {
        if (image->align == ESCPOS_ALIGN_CENTER) {
            left_offset = (uint16_t)((area_width - effective_width) / 2);
        } else if (image->align == ESCPOS_ALIGN_RIGHT) {
            left_offset = (uint16_t)(area_width - effective_width);
        }
    }

    /* TEMPORARY: Skip alignment commands - may be causing issues */
    /*
    esp_err_t ret = escpos_write_raw(printer, align_cmd, sizeof(align_cmd));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set image alignment: %s", esp_err_to_name(ret));
        return ret;
    }

    if (left_offset > 0) {
        pos_cmd[2] = (uint8_t)(left_offset & 0xFF);
        pos_cmd[3] = (uint8_t)((left_offset >> 8) & 0xFF);
        ret = escpos_write_raw(printer, pos_cmd, sizeof(pos_cmd));
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to set image print position: %s", esp_err_to_name(ret));
            return ret;
        }
    }
    */
    ...
    /* GS v 0 expects image width in bytes (x-byte-count), not dots. */
    uint32_t idx = 0;
    header[idx++] = 0x1D;  /* GS */
    header[idx++] = 0x76;  /* v */
    header[idx++] = 0x30;  /* 0 */
    ...
    uint16_t width_bytes = (uint16_t)bitmap_row_bytes;
    header[idx++] = (width_bytes & 0xFF);        /* xL */
    header[idx++] = ((width_bytes >> 8) & 0xFF); /* xH */
    ...
    ret = escpos_write_raw(printer, header, sizeof(header));
    if (ret != ESP_OK) {
    ...
    for (uint16_t row = 0; row < image->height; row++) {
        ...
        ret = escpos_write_raw(printer, row_data, bitmap_row_bytes);
        if (ret != ESP_OK) {
    ...
    ESP_LOGI(TAG, "Image sent to printer (%dx%d, %zu bytes, %zu bytes/row, align=%d, offset=%u/%u dots)",
             image->width, image->height, expected_bitmap_size, bitmap_row_bytes,
             image->align, left_offset, area_width);

    /* TEMPORARY: Skip alignment reset commands */
    /*
    align_cmd[2] = ESCPOS_ALIGN_LEFT;
    ret = escpos_write_raw(printer, align_cmd, sizeof(align_cmd));
    if (ret != ESP_OK) {
        return ret;
    }
    ...
    pos_cmd[2] = 0;
    pos_cmd[3] = 0;
    return escpos_write_raw(printer, pos_cmd, sizeof(pos_cmd));
    */
    return ret;
}
